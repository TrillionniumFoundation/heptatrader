#include "heptaMarKetDataReceiverToLocalKindle.h"
#include "heptaTimeStamp.h"
#include "csvparser.h"
#include <fstream>


heptaMarKetDataReceiverToLocalKindle::heptaMarKetDataReceiverToLocalKindle()
...
std::string heptaMarKetDataReceiverToLocalKindle::GetStrategyName()
{
	std::string strStrategyName("heptaMarKetDataReceiverToLocalKindle");
...
void heptaMarKetDataReceiverToLocalKindle::PriceUpdate(heptaMarketDataPtr pPriceData)
{
	//heptaMarketDataReceiver::PriceUpdate(pPriceData);


}

void heptaMarKetDataReceiverToLocalKindle::OnBar(heptaMarketDataPtr pPriceData, int iTimeScale, heptaBasicKindleStrategy::heptaKindleSeriesPtr pKindleSeries)
{
	heptaAUTOMUTEX mt(m_DequeMutex, true);
...
void heptaMarKetDataReceiverToLocalKindle::OnReady()
{
	SetWriteIndexInfoCacheToFile(true);

	//heptaMarketDataReceiver::OnReady();
	...
	if (!m_bWorkingThreadRun)
	{
		...
		m_WorkingThread = std::thread(&heptaMarKetDataReceiverToLocalKindle::WorkingThread, this);
	}

}

void heptaMarKetDataReceiverToLocalKindle::InitialStrategy(const char * pConfigFilePath)
{
	heptaMarketDataReceiver::InitialStrategy(pConfigFilePath);
...
bool heptaMarKetDataReceiverToLocalKindle::InitialHisKindleFromHisKindleFolder(const char* szHisFolder)
{
	heptaBasicKindleStrategy::InitialHisKindleFromHisKindleFolder(szHisFolder);
...
void heptaMarKetDataReceiverToLocalKindle::WorkingThread()
{
	{
		//创建文件夹
		struct stat buf;
		...
		{
			if (HEPTA_FTDC_PC_Futures != it->second->ProductClass)
			{
				continue;
			}
			...
			SubcribeKindle(it->second->InstrumentID, heptaKINDLE_TIMESCALE_1MIN);

			//SubcribeIndexPriceKindle

			auto pKindle = SubcribeIndexKindle(it->second->ProductID, heptaKINDLE_TIMESCALE_1MIN);
	...
	while (m_bWorkingThreadRun)
	{
			...
			it != m_KindleFinishedIndex.end(); it++)
		{
				...
				&& FileIt->second.size() != 0)
			{
				...
				if (m_Log.good())
				{
					...
					auto pKindleSeries = GetKindleSeries(it->first.c_str(), heptaKINDLE_TIMESCALE_1MIN);
					if (pKindleSeries.get() != nullptr)
					{
						std::deque<heptaKindleStickPtr> KindleStickDeque;
							...
							i <= it->second.Finished; i++)
						{
							...
							heptaKindleStickPtr pKindle = KindleStickDeque.at(i);
		...
		for (auto it = m_KindleFinishedIndex.begin();
			it != m_KindleFinishedIndex.end(); it++)
		{
			if (it->second.NothingToWriteCount > 2 * heptaKINDLE_TIMESCALE_1MIN)
			{
