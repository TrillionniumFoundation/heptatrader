...
class heptaChasingRisingStrategy :
...
public:
	...
	struct StrategyParameter
	{
		...
		heptaOpenCloseMode OpenCloseMode;		//开平模式
		...
		StrategyParameter()
			...
			, OpenCloseMode(heptaOpenCloseMode::CloseTodayThenYd)
			, OpenCancelLimit(350)
	...
	{
		heptaMarketDataPtr LastMarketData;
	...
	typedef std::shared_ptr<RunningParameter>			heptaRunningParaPtr;


	StrategyParameter									m_heptaStrategyParameter;
	heptaRunningParaPtr									m_heptaRunningParaPtr;

	std::map<std::string, StrategyParaPtr>				m_StrategyParameterMap;		//key Instrument value:StrategyParameter
	std::map<std::string, heptaRunningParaPtr>				m_heptaRunningParameterMap;	//key Instrument value Running Parameter
	...
	int64_t TimeToint64(heptaMarketDataPtr pData);
