...
class heptaTimeStamp
{
public:
	enum heptaWeekday :int
	{
		Sunday = 0,			//周日
		Monday = 1,			//周一
		Tuesday = 2,		//周二
...
public:
	heptaTimeStamp(uint64_t totalMicrosecond = 0L);
	heptaTimeStamp(std::uint32_t year, std::uint32_t month, std::uint32_t day,
	...
	bool operator > (heptaTimeStamp const& right) const;
	bool operator < (heptaTimeStamp const& right) const;
	bool operator >= (heptaTimeStamp const& right) const;
	bool operator <= (heptaTimeStamp const& right) const;
	bool operator == (heptaTimeStamp const& right) const;
	bool operator != (heptaTimeStamp const& right) const { return !operator ==(right); }
	
	std::int64_t operator - (heptaTimeStamp const& right) const;
	heptaTimeStamp operator +(std::uint64_t offset) const;
	heptaTimeStamp operator -(std::uint64_t offset) const;
	heptaTimeStamp operator +=(std::uint64_t offset);
	heptaTimeStamp operator -=(std::uint64_t offset);
