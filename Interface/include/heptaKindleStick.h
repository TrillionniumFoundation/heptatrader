...
public:
	...
	{
		return sizeof(heptaKindleStick);
	}
	...
	inline double GetPriceByType(KindlePriceType type)
	{
		...
		switch (type)
		{
		case heptaKindleStick::en_Open:
			price = Open;
			break;
		case heptaKindleStick::en_High:
			price = High;
			break;
		case heptaKindleStick::en_Low:
			price = Low;
			break;
		case heptaKindleStick::en_Close:
			price = Close;
			break;
		default:
			break;
	...
	heptaKindleStick(std::uint64_t start, std::uint64_t end, double low, double high, double open, double close,
