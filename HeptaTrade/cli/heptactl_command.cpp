...
bool ApplyField(const std::string& argument, HeptaCtlCommand& command, std::string& reason)
{
    ...
    else if (key == "primary_exchange") call.ibContract.primaryExchange = value;
    else if (key == "contract_month") call.ibContract.lastTradeDateOrContractMonth = value;
    else if (key == "right") call.ibContract.right = value;
    else if (key == "multiplier") call.ibContract.multiplier = value;
    else if (key == "trading_class") call.ibContract.tradingClass = value;
    else if (key == "local_symbol") call.ibContract.localSymbol = value;
    else if (key == "side") call.ibOrder.action = value;
    else if (key == "order_type") call.ibOrder.orderType = value;
    else if (key == "position_effect") call.ibOrder.positionEffect = value;
    ...
    else if (key == "strike" && ParseDouble(value, decimal)) call.ibContract.strike = decimal;
