...
const char* ProposalSetBuilder::Version()
{
    return "proposal-set.v1";
}
...
std::string ProposalSetBuilder::Digest(const ProposalSet& proposalSet)
{
    ...
    AppendField(canonical, "captured_at_ms",
                std::to_string(proposalSet.capturedAtMs));
    for (std::size_t i = 0; i < proposalSet.proposals.size(); ++i)
    {
        AppendField(canonical, "module_id",
                    proposalSet.proposals[i].moduleId);
        AppendField(canonical, "proposal_id",
                    proposalSet.proposals[i].proposalId);
        AppendField(canonical, "proposal_digest",
                    proposalSet.proposals[i].proposalDigest);
    }
...
ProposalSetBuildResult ProposalSetBuilder::Build(
    ...
    std::uint64_t nowMs)
{
    if (nowMs == 0 || expectedModules.empty() || expectedModules.size() > 256u)
    ...
    if (proposals.size() != expected.size())
        return Reject("PROPOSAL_SET_INCOMPLETE");
