...
def _markdown_files(root: Path, modules: dict[str, dict[str, Any]]) -> list[Path]:
    ...
    files.append(root / "docs/DEVELOPMENT-DOCUMENTATION-INDEX.md")
    return files
...
INDEX_BEGIN = "<!-- module-catalog:begin -->"
INDEX_END = "<!-- module-catalog:end -->"


def render_module_table(modules: dict[str, dict[str, Any]]) -> str:
    """Generate navigation from the catalog, not a second completion score."""
    lines = [INDEX_BEGIN, "| Module | Status | Development document |", "|---|---|---|"]
    for module_id, module in modules.items():
        target = Path(module["document"]).relative_to("docs").as_posix()
        lines.append(f"| `{module_id}` | {module['status']} | [{target}]({target}) |")
    lines.append(INDEX_END)
    return "\n".join(lines)


def validate_generated_index(root: Path, modules: dict[str, dict[str, Any]]) -> list[str]:
    text = _read_text(root / "docs/index.md", "docs/index.md", [])
    if text.count(INDEX_BEGIN) != 1 or text.count(INDEX_END) != 1:
        return ["docs/index.md: exactly one generated module table is required"]
    start, end = text.index(INDEX_BEGIN), text.index(INDEX_END) + len(INDEX_END)
    if text[start:end] != render_module_table(modules):
        return ["docs/index.md: module navigation drift; run check_documentation.py --write-index"]
    return []
...
def write_index(root: Path) -> None:
    ...
    if text.count(INDEX_BEGIN) != 1 or text.count(INDEX_END) != 1:
        raise ValueError("index must contain one begin/end module table marker pair")
    start, end = text.index(INDEX_BEGIN), text.index(INDEX_END) + len(INDEX_END)
    if end <= start:
        raise ValueError("module table markers are reversed")
...
def validate(root: Path | str = ROOT) -> list[str]:
    ...
    errors.extend(validate_generated_index(root, modules))
    return errors
