...
FIXTURE = r"""#include "features/feature_graph.h"
...
void TestStrategyRuntime()
{
    ...
    descriptor.budget.maxMemoryBytes = 64 * 1024 * 1024;
    descriptor.budget.maxCheckpointBytes = 1024 * 1024;

    StrategyRuntimeControlResult admitted = control.Admit(descriptor, 100);
    ...
    StrategyRuntimeControlResult running =
    ...
    StrategyRuntimeControlResult duplicate =
    ...
    StrategyRuntimeControlResult quarantined =
    ...
    StrategyRuntimeControlResult replaced =
...
void TestDurableRollout(const std::filesystem::path& directory)
{
    ...
    DurableRolloutResult duplicate = store.Put(record);
    assert(duplicate.accepted && duplicate.duplicate);
...
void TestScenario()
{
    ...
    MultiAgentScenarioResult sealed = scenario.Seal();
    assert(sealed.accepted && !sealed.scenarioDigest.empty());

    MultiAgentScenarioResult first = scenario.AdvanceTo(15);
    ...
    MultiAgentScenarioResult second = scenario.AdvanceTo(30);
    ...
    MultiAgentScenarioResult replay = scenario.AdvanceTo(30);
...
void TestFeatureGraph()
{
    ...
    FeatureGraphResult first = graph.Evaluate({{"x", 10}, {"y", 2}}, 1);
    assert(first.accepted && first.reasonCode == "FEATURE_GRAPH_WARMING");
    assert(!first.values["mean"].ready);
    FeatureGraphResult second = graph.Evaluate({{"x", 20}, {"y", 4}}, 2);
    assert(second.accepted && !second.values["mean"].ready);
    FeatureGraphResult third = graph.Evaluate({{"x", 30}, {"y", 6}}, 3);
    assert(third.accepted && third.values["mean"].ready);
    assert(third.values["mean"].value == 24);
...
class BoundedRuntimeComponentsTests(unittest.TestCase):
    def test_components_compile_and_fail_closed(self) -> None:
        compiler = shutil.which("g++")
        self.assertIsNotNone(compiler, "g++ is required for bounded runtime verification")
        with tempfile.TemporaryDirectory() as directory:
            temporary = Path(directory)
            source = temporary / "bounded_runtime_components.cpp"
            binary = temporary / "bounded_runtime_components"
