...
The implementation remains Linux-oriented, matching the canonical deployment target. These integrity guarantees have no PAPER or LIVE authorization effect; external governance and Broker qualification remain separate requirements.

## Verification

`tests/python/test_release_security_races.py` deterministically covers competing package and receipt publishers, payload mutation after snapshot, and archive pathname replacement during preflight. The ordinary release, preflight, CMake install, documentation, ownership, Python and C++ suites continue to run on the exact pull-request head.
