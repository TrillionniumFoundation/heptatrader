...
Feature is constructed against one exact Market Data authority. Receipt issuance and consumption acquire the source shard before sampling authority time, so a request cannot wait in a queue and then reuse a pre-wait timestamp after expiry. Before every authoritative Feature write, the issuer revalidates the receipt against its current entry and retains the source-shard plus lifecycle lock until the derived Feature state is committed. The validation and Feature write therefore have one explicit linearization interval: the source cannot advance between current-state validation and an accepted cache write. Before every risk-ready Feature read, the source lineage is revalidated again.

The default authority clock is wall-time anchored to a monotonic steady clock; a test or platform clock may be injected only as a trusted, non-reentrant provider. Any observed rollback faults the complete lifecycle epoch. Restoring the numeric clock value does not reopen authority: an explicit fence and fresh consumer binding are required. Cross-store transfer, reconstructed-store substitution, issuer destruction, generation advance, gap-after-issuance, clock regression, expiry and replay into a fresh Feature store fail closed. A future cross-process deployment requires a separately authenticated issuer/audience/nonce/expiry envelope; the same-process receipt is never serialized as proof.
