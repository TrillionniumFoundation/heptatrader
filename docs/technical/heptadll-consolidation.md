...
| External order-flow book | Explicit price/time orders/cancels, queue-ahead, GTC, self-trade prevention | Not implemented | Not implemented | Ported as the named `OrderFlowReplay` input/model contract using shared accounting; no depth-to-queue inference or second framework |
| Cumulative tick turnover decomposition | Historical heuristic price-volume map and bid/ask-side totals | Missing | Missing | Port only the uniquely solvable previous-one-tick-book case as `CumulativeTopOfBookTradeInference`; the four reviewed legacy Tick layouts expose Bid1/Ask1 only through an explicit typed Data adapter and fixed Data→Replay binding; wider-spread/multi-price and venue-specific heuristics remain retained |
...
| Cumulative volume/turnover inference | Native `CumulativeTopOfBookTradeInference`: consecutive counters + previous adjacent bid/ask yield a unique bounded two-price split, marked inferred; reviewed legacy Tick sources use `DecodeLegacyTopOfBook` / `LegacyCumulativeTradeObservation` rather than consumer-owned positional parsing | Historical multi-price reconstruction, venue-specific special cases, statistics/log files, displayed-size queue inference and any live-feed authority remain retained |
| Five-level legacy depth | `DecodeLegacyDepth5` maps the reviewed four CSV layouts into one typed, validated offline snapshot with contiguous levels and monotone prices | Observation only: no order-book owner, queue reconstruction, own-order deduction, hidden liquidity, matching or production feed path |
| Legacy performance vocabulary | Existing `EvaluateEquity` extends its explicit-flow/explicit-sampling contract with average drawdown, annualized downside deviation and Sterling alongside Sharpe/Sortino/Calmar | Legacy 16:00/weekend/natural-year assumptions and exact arithmetic/ABI are deliberately not reproduced |
