...
| Cumulative tick turnover decomposition | Historical heuristic price-volume map and bid/ask-side totals | Missing | Missing | Port only the uniquely solvable previous-one-tick-book case as `CumulativeTopOfBookTradeInference`; wider-spread/multi-price and venue-specific heuristics remain retained |
...
| Cumulative volume/turnover inference | Native `CumulativeTopOfBookTradeInference`: consecutive counters + previous adjacent bid/ask yield a unique bounded two-price split, marked inferred | Historical multi-price reconstruction, venue-specific special cases, statistics/log files and any live-feed authority remain retained |
