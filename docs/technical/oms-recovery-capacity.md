...
The opt-in installed process lane additionally executes the real simulator across fill → stop → V2 seal → restart and verifies economic position, command identity, duplicate no-resend and order-ID watermark. The recovery-growth fixture remains useful for measuring legacy full-ledger cost. Generation fixtures prove bounded coordinator hot restart and bounded maintenance working memory, not bounded total disk storage. The source suite also records a synthetic generation-cost curve across increasing retained history (seal/verify time, retained bytes, rebase time and process RSS), while the isolated installed-process lane records actual simulator restart recovery latency and per-process Linux `VmHWM` at increasing admitted-order counts plus a post-rebase restart. These measurements are descriptive evidence from the exact CI host, not fixed performance thresholds. None of these source fixtures is a target-host multiday soak, physical durability benchmark, Broker qualification campaign or proof that a chosen maintenance cadence satisfies an operational SLO.
