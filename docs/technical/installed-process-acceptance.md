...
The same process lane also measures bounded authenticated quote reads, checks warmed descriptor counts, executes four persisted-position restart cycles, and restores an offline checkpoint into a new private fixture without re-provisioning or resending the recovered command. Its generation cost curve is retained as create-only `process-evidence/installed-generation-cost-curve.json`; the record binds candidate source/package identity and carries staged recovery, simulator recovery, complete startup readiness, VmHWM, place p99 bucket, seal/storage and post-rebase observations. The shared acceptance driver rejects a missing, identity-drifted or structurally invalid curve before issuing PASS. Set `HEPTA_SOAK_SAMPLES=1000` for a 1000-call observation; the default is 256. See [clock/load/restore acceptance](bounded-runtime-acceptance.md) for the measurement boundary, backup trust scope and limits. These are bounded simulator tests, not a long-duration production certification.
