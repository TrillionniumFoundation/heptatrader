...
| CSV/data helpers | Strict portable tick/session/completed-bar CSV, four selected legacy Tick layouts and three selected completed-bar layouts with default or explicitly declared positive duration through the SAME Data SDK. Mixed Tick instruments, explicit source clocks, correct per-bar amounts, original-column retention and independent completion/observation evidence are supported. The four reviewed Tick layouts also have an explicit opt-in typed Bid1/Ask1 adapter; ordinary reads keep depth columns unqualified. Proprietary binary cache layouts and historical datasets remain in the source repository. |
