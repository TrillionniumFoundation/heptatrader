...
| CSV/data helpers | Strict portable tick/session/completed-bar CSV, four selected legacy Tick layouts and three selected completed-bar layouts with default or explicitly declared positive duration through the SAME Data SDK. Mixed Tick instruments, explicit source clocks, correct per-bar amounts, original-column retention and independent completion/observation evidence are supported. The four reviewed Tick layouts have opt-in typed Bid1/Ask1 and five-level snapshot adapters. Depth stays offline observed input: no queue reconstruction, own-order deduction, hidden-liquidity inference or executable-liquidity claim. Proprietary binary cache layouts and historical datasets remain in the source repository. |
| `heptaNetValueEvaluation`, `heptaSettlement` | Research-only cash-flow-adjusted metrics and multiplier-aware P&L ledger with explicit undefined ratios, fees, fill identity and offline variation-settlement events. Analytics preserves max/average drawdown, annualized downside deviation, Sharpe/Sortino/Calmar/Sterling vocabulary while retaining canonical explicit sampling/cash-flow/risk-free inputs instead of the legacy implicit 16:00/weekend/calendar assumptions. No formula/ABI parity or full exchange settlement/margin equivalence is asserted. |
...
| `heptaTickTradeManager` | `CumulativeTopOfBookTradeInference` ports only the uniquely solvable cumulative-volume/turnover arithmetic for a caller-evidenced previous one-tick bid/ask. `DecodeLegacyTopOfBook` plus `LegacyCumulativeTradeObservation` provide the reviewed four-layout Data→Replay field binding without reconstructing depth or queue state. Output is explicitly inferred and research-only. Wider-spread/multi-price reconstruction, venue-specific heuristics, statistics/logging and live-feed behavior remain retained rather than silently emulated. |
