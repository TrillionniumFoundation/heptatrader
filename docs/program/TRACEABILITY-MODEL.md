...
任何 capability 如果缺少 module、contract、verification 或 maturity/qualification 映射，只能是 `planned` 或 `unsupported`。任何 current module 如果没有 owner、backup、state/concurrency/failure/resource contract，或者没有可追踪到真实源文件、构建目标和验证 ID 的完整技术指南，不得作为独立团队交付面。
