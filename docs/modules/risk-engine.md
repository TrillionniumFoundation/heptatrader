...
The deterministic simulator now calls `BuildExposure` inside its existing venue mutex, using the complete existing EUR.USD/GBP.USD CASH/USD universe, explicit zero positions, actual inert/active reservations, retained pending contract identity and fresh Execution-owned quotes. Position and pending aggregation use the same checked implementation as `Build`; the shared risk target supplies it to the real simulator runtime and tests. Risk-increasing monetary admission requires fresh marks for both declared instruments, even a zero position. Unknown restored/pending instruments and unsupported pending contract units reject rather than disappear from the aggregate. The existing strictly reducing flatten-only path remains independent of unrelated portfolio marks; its own quote, quantity reservation, kill/rate/price checks and no-crossing proof are unchanged.

`BuildExposure` deliberately leaves PnL/equity sections absent and does not consume account evidence. `Build` still requires the independently complete account observation described above. No synthetic zero PnL/equity is created; simulator risk increase with a configured daily-loss or drawdown limit remains rejected until those facts have a real owner. This is simulator product wiring, not a new Broker authority or a generalized multi-asset qualification. The IB PAPER profile remains one active order / one CASH quote contract. Margin consumption, real multi-asset Broker producers, account accounting and portfolio telemetry remain required before broader qualification.
