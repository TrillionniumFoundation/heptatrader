...
Flatten-only admission reserves reducible quantity separately from monetary exposure. Every unresolved order on the same instrument in the reduction direction consumes capacity, including ordinary orders admitted before a policy change, unactivated reservations and cancellation requests awaiting a terminal outcome. Opposite-side pending orders cannot increase that capacity. Thus a position of ten permits exits of four and six, but not two exits of ten. Partial or exact exits use strict floating-point subtraction; a nominal decimal sum that exceeds the representable remainder is rejected without a tolerance that could authorize crossing zero.

An order admitted under flatten-only retains its zero boundary if the policy is later disabled. At fill time, both those orders and any order subject to the current flatten-only policy must strictly reduce the actual position without crossing zero. If another fill consumed the capacity, the venue emits one terminal `Rejected` event with no economic fill. This also protects ordinary orders admitted before flatten-only was enabled.
...
Quantity reservations belong to those in-memory active orders and are not replayed as fictitious live orders. Within a running venue, `RestoreRiskState` refuses to replace positions or admission counts after any order has been admitted, so restoring a baseline cannot erase an active reservation.
