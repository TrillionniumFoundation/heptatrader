...
Leaf inputs are opened nonblocking and no-follow, rejected from the acquired descriptor unless they are stable regular single-link files, and switched back to ordinary blocking reads only after admission. This prevents FIFOs and pathname exchanges from blocking before validation. Broker TCP probing is unreachable until both policy loading and complete artifact inspection have succeeded; a failed, missing, malformed, substituted, or special-file artifact can never cause network I/O.
...
The special-file suite uses timeout-bounded subprocesses for FIFO policy, artifact, installed leaf, wrapper core and regular-to-FIFO builder exchanges. The main preflight suite includes a discovery meta-test for every declared regression and mocks `socket.create_connection` to prove rejected artifacts cannot initiate a Broker probe.
