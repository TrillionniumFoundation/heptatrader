...
The source repository still cannot create GitHub organization teams, branch rulesets, protected environments, runner assignments, Broker credentials, TWS/IB Gateway sessions or a PAPER account. Static preflight deliberately stops before those external trust domains. The protected qualification workflow must bind its Broker-observed receipt to the unchanged package digest before PAPER authorization can change.
