...
Stopped-state generation maintenance is explicit. It does not run concurrently with the writer, expire command IDs, grant PAPER/LIVE authority or make a damaged generation ignorable. Ordinary V2 seals immutable deltas, carry a digest-bound simulator economic checkpoint, rotate the active journal to a lineage-bound tail, and provide explicit strict legacy export for downgrade. The active tail is bounded by maintenance cadence. Explicit stopped-state `rebase` reconstructs the exact ledger into one independent V2 root and may prune only the previously selected ancestor chain after the new root and active tail verify; command identity, every possible send and downgrade history remain lossless.
...
The simulator uses two-phase placement: reserve an inert order, establish its owner and projection, durably append `place_sent` with status `activation_pending`, then activate it and durably append `place_activated`. Reserved orders count toward pending risk but cannot submit or fill. A crash without the final activation receipt replays as uncertain. Activation failure or exception appends a later critical `place_outcome_uncertain`, fences mutations, and survives replay; the pending receipt cannot overwrite that uncertainty. V2 seals also project cumulative simulator position, admitted-order count and order-ID watermark into the bounded hot generation state, so normal generation-backed restart restores that checkpoint and only applies the active tail instead of rebuilding maps for every historical order. Immediate broker adapters leave the optional activation callback unset.
...
Generation V2 bounds the active replay working set and historical command cache, not every long-horizon operation. Ordinary seals retain cumulative index snapshots and therefore still consume increasing disk until an explicit stopped-state rebase compacts the selected lineage. A first historical send-window query after generation selection, an account/domain change, or a backwards cutoff performs an exact indexed historical query before subsequent forward cutoffs reuse the bounded suffix. Generation-backed terminal PAPER finalization streams the sealed owner/session/account/domain history into a compact binding and combines only the active tail; the manifest remains a qualification/finalization artifact, not an unlimited production-history ledger.
