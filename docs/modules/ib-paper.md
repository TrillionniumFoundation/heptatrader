...
The installed `hepta_ib_runtime_report.py` validates the fixed schema and exports fixed-cardinality Prometheus gauges without reason strings as labels. It exports real callback queue-lag histograms and callback-conflict counters from the adapter, primary-quote and complete authoritative-snapshot ages from the runtime, and a native successful post-fill reconciliation-duration histogram. Missing/invalid ages carry explicit validity rather than numerical zero. Network-policy observation remains absent until an actual host nftables readback producer supplies it. Actual target-host collection/notification is separate external evidence; source metrics do not authorize PAPER.
