# CTP Venue Adapter Technical Guide

Status: generated current view
Applies to: `hepta.venue.ctp` version `1.0.0` (unsupported)
Verification: `python3 scripts/generate_documentation_views.py --check`
Authority: generated from `modules/manifests/hepta-venue-ctp.json`, module-documentation-profiles-v1.json and canonical registries

> 本文件由机器注册表确定性生成。请修改注册表，不要直接修改本文件。

Manifest: [`modules/manifests/hepta-venue-ctp.json`](../manifests/hepta-venue-ctp.json)
...
- CTP protocol integration
- PAPER operation
- LIVE operation

### Direct implementation evidence

- **Source evidence:** `HeptaTrade/adapter_ctp/`
...
Defines the quarantined adapter boundary for a future CTP integration; the current lifecycle does not authorize runtime use.
...
- Does not expose an active CTP connection or credentials.
...
- **Source roots:** `HeptaTrade/adapter_ctp/`
- **Build targets:** `hepta_venue_ctp`
