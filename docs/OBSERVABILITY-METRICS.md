...
Complete per-reason execution lifecycle beyond coordinator call outcomes,
portfolio notional/PnL/drawdown and reconnect/refresh duration still need
individually bounded producers and behavior tests. Callback queue lag/conflict,
primary quote age, authoritative snapshot age, successful post-fill Broker
reconciliation duration, and exact canonical/deny-all Broker network-policy
readback are now source-produced. A
