...
|---|---|---|
| Gateway activity and installed report | actual Unix server + daemon JSON + `hepta_gateway_report.py` | ingress/queue/dispatch/reply histograms, fixed result counts, write failures, queue/worker gauges and backpressure; [contract](technical/gateway-operational-observations.md); real IPC and installed-process tests |
...
Execution-side per-reason lifecycle
