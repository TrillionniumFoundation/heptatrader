...
Tests: `tests/python/test_release_package.py`, `tests/python/test_release_security_races.py`, `tests/python/test_hepta_preflight.py`
...
The release security race suite injects the following deterministic interleavings:
...
Passing these checks proves only byte identity, deterministic packaging and local preflight integrity. It does not create GitHub governance controls, Broker credentials, PAPER qualification, or LIVE authority. PAPER and LIVE authorization remain governed by the capability and external qualification records.
