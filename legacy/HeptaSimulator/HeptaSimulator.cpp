// heptaPegasusSimulator.cpp : Defines the entry point for the  Hepta Simulator console application.
//
...
#include "heptaPegasusSimulator.h"

#ifdef REALTIME_QUOTE
#include "heptaFtdMdSpi.h"
#else
#include "heptaSimMdSpi.h"
#endif // REALTIME_QUOTE

#include "heptaSimTradeSpi.h"
...
heptaPegasusSimulator		m_PegasusSimulator;

#ifdef REALTIME_QUOTE
heptaFtdMdSpi				m_mdCollector;
#else
heptaSimMdSpi				m_mdCollector;
#endif // REALTIME_QUOTE

heptaSimTradeSpi			m_TradeChannel;
heptaStrategyDemo			m_Strategy;
...
int main()
{
	m_heptaShow.AddLog("Welcome To Hepta Simulator !!");
...
#ifdef WIN32
	strFullPath.append("\\HeptaSimulatorConfig.xml");
#else
	strFullPath.append("/HeptaSimulatorConfig.xml");
#endif // WIN32
...
#ifdef REALTIME_QUOTE
	m_PegasusSimulator.SetMdSpi((void*)dynamic_cast<heptaBasicMdSpi*>(&m_mdCollector));
#else
	...
	m_TradeChannel.RegisterBasicStrategy(dynamic_cast<heptaBasicStrategy*>(&m_Strategy));

	m_mdCollector.RegisterTradeSPI(dynamic_cast<heptaBasicTradeSpi*>(&m_TradeChannel));
	m_mdCollector.RegisterStrategy(dynamic_cast<heptaBasicStrategy*>(&m_Strategy));
	...
	{
		heptaSleep(3000);
		m_heptaShow.AddLog("%s %s 权益:%.3f 平仓盈亏:%.3f 持仓盈亏:%.3f",
			m_PegasusSimulator.m_CurrentTradingDay,
			m_PegasusSimulator.m_CurrentSimulationTime,
			m_PegasusSimulator.m_heptaSettlement.m_dBalance,
			m_PegasusSimulator.m_heptaSettlement.m_dCloseProfit,
			m_PegasusSimulator.m_heptaSettlement.m_dPositionProfit);
	}
    return 0;
}
