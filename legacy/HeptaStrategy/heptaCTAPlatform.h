...
#include "heptaDualTrust.h"


class heptaCTAPlatform :
...
public:
	...
	enum heptaStrategyEnum:int
	{
		hepta_DualTrust_Strategy = 0
		, heptaStrategyCount
	};
	...
	{
		heptaKindleSeriesPtr pKindleSeries;
	};
...
#pragma region CommenDefine
	...
	struct TradeParameter
	{
		...
		heptaOpenCloseMode InsOpenCloseMode;		//开平模式
		...
		TradeParameter()
			...
			, Ratio(0)
			, Mod(true)
			, InsOpenCloseMode(heptaOpenCloseMode::CloseTodayThenYd)
...
#pragma endregion
	...
	{
		heptaBasicCTAStrategy	*	_pStrategy;
...
public:
	...
	bool					AddStrategyToPools(std::string strStrategyID, heptaBasicCTAStrategy * pCTAStrategy, StrategyParaPtr pPara);
	...
	heptaInstrumentDataPtr		GetFirstInstrumentData(std::string ProductID);
...
protected:
	...
	std::map<std::string, std::map<std::string, double>> m_heptaStrategyPositionMap;
	...
	std::map<std::string, heptaHeptaAgentManager::heptaAgentDataPtr>	m_heptaAgentDataMap;			//key Instrument value AgentData
