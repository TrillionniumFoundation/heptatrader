...
heptaNetValueEvaluation::heptaNetValueEvaluation()
    : m_iTimeStamp(0)
    , m_iStartTimeStamp(0)
    , m_dPreTotalPNL(0.0)
    , m_dLastEquity(0.0)
...
void heptaNetValueEvaluation::Deposit(double deposit)
{
...
void heptaNetValueEvaluation::UpdateNetValueByTotalPNL(std::uint64_t timestamp, double totalPNL, double fundUsed)
{
    // 更新时间
    ...
    if (m_iStartTimeStamp == 0 && fundUsed > 0) {
        ...
        heptaTimeStamp tEndDay(timestamp);
        tEndDay.SetHour(16);
        tEndDay.SetMinute(0);
        tEndDay.SetSecond(0);
        ...
        while (tEndDay.GetDayofWeek() == heptaTimeStamp::Sunday || tEndDay.GetDayofWeek() == heptaTimeStamp::Saturday) {
            tEndDay.GoForward(1);
        }
...
void heptaNetValueEvaluation::Calculate()
{
...
void heptaNetValueEvaluation::SetExpectedReturn(double expectedret)
{
