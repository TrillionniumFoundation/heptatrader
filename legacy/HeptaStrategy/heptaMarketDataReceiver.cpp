...
heptaMarketDataReceiver::heptaMarketDataReceiver()
...
std::string heptaMarketDataReceiver::GetStrategyName()
{
...
void heptaMarketDataReceiver::PriceUpdate(heptaMarketDataPtr pPriceData)
{
	...
	m_StrategyLog.AddLog(heptaStrategyLog::enIMMS,
...
void heptaMarketDataReceiver::OnBar(heptaMarketDataPtr pPriceData, int iTimeScale, heptaBasicKindleStrategy::heptaKindleSeriesPtr pKindleSeries)
{
}


void heptaMarketDataReceiver::InitialStrategy(const char * pConfigFilePath)
{
...
void heptaMarketDataReceiver::OnReady()
{
	...
	for (auto it = m_InstrumentMap.begin(); it != m_InstrumentMap.end(); it++)
	{
		if (HEPTA_FTDC_PC_Futures == it->second->ProductClass
			|| HEPTA_FTDC_PC_Options == it->second->ProductClass
			)
		{
