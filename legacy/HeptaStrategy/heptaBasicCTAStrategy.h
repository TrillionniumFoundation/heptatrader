...
#include "heptaNetValueEvaluation.h"

//#define HEPTA_NEED_STRATEGY_LOG

class heptaBasicCTAStrategy
{
	//
public:
	struct heptaCTAParaField
	{
		union
		{
			double			ParaList[50];
...
public:
	heptaBasicCTAStrategy(const char* szStrategyName);
	~heptaBasicCTAStrategy();

	//初始化策略
	virtual void			InitialStrategy() {};
	//当生成一根新K线的时候，会调用该回调
	virtual void			OnBar(bool bFinished, int iTimeScale, heptaBasicKindleStrategy::heptaKindleSeriesPtr pKindleSeries) = 0;
	...
	heptaCTAParaField			m_StrategyPara;
	std::string				m_strDealInstrument;

	heptaInstrumentDataPtr		m_pInstrument;
	//
	void					_PreOnBar(bool bFinished, int iTimeScale, heptaBasicKindleStrategy::heptaKindleSeriesPtr pKindleSeries);
...
private:
	heptaStrategyLog			m_StrategyTradeListLog;
...
public:
	...
	//用于记录策略净值变化
	heptaSettlement									m_heptaSettlement;
	std::deque<TimeBalanceDataPtr>					m_dTimeBalanceDQ;//采用push back方法增加元素
	

	//策略评价器
	heptaNetValueEvaluation                            m_heptaEvaluator;
