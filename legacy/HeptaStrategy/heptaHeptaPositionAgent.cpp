...
heptaHeptaPositionAgent::heptaHeptaPositionAgent()
{
	m_bAgentWorking = true;

	m_iExpectPosition = 0;

	OpenCloseMode = heptaBasicStrategy::heptaOpenCloseMode::CloseTodayThenYd;
...
void heptaHeptaPositionAgent::PriceUpdate(heptaMarketDataPtr pPriceData)
{
...
void heptaHeptaPositionAgent::OnRtnTrade(heptaTradePtr pTrade)
{
	if (m_bAgentWorking)
	{
		heptaProductTradeTime::heptaTradeTimeSpace TradeTimeSpace = heptaProductTradeTime::NoTrading;
...
void heptaHeptaPositionAgent::OnRtnOrder(heptaOrderPtr pOrder, heptaOrderPtr pOriginOrder)
{
	if (m_bAgentWorking)
	{
		heptaProductTradeTime::heptaTradeTimeSpace TradeTimeSpace = heptaProductTradeTime::NoTrading;
...
void heptaHeptaPositionAgent::OnOrderCanceled(heptaOrderPtr pOrder)
{
}

void heptaHeptaPositionAgent::OnRspOrderInsert(heptaOrderPtr pOrder, heptaRspInfoPtr pRspInfo)
{
}

void heptaHeptaPositionAgent::OnRspOrderCancel(heptaOrderPtr pOrder, heptaRspInfoPtr pRspInfo)
{
}

void heptaHeptaPositionAgent::SetExpectPosition(int iExpPos)
{
...
void heptaHeptaPositionAgent::DealExpectedPosition(std::string InstrumentID, int iExpectedMaintain/*= 0*/, const char * szCallMsg /*= NULL*/)
{
	...
	heptaMarketDataPtr MarketData = GetLastestMarketData(InstrumentID);
		...
		WaitOrderIt != WaitOrderList.end(); WaitOrderIt++)
	{
		...
		{
			if (HEPTA_FTDC_D_Buy == WaitOrderIt->second->Direction)
			{
		...
		WaitOrderIt != WaitOrderList.end(); WaitOrderIt++)
	{
		if (InstrumentID == (std::string)WaitOrderIt->second->InstrumentID)
		{
			...
			if (HEPTA_FTDC_D_Sell == WaitOrderIt->second->Direction)
			{
				int iLittleVol = InsLittleOrderVolume;
				if (MarketData->BidPrice1 - MarketData->LowerLimitPrice < dInsEQ)
				{
