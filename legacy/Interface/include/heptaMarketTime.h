...
class heptaMarketTime
{
public:
	//UpdateTime Format: hh:mm:ss 09:10:11
	heptaMarketTime(int StartTime = 6);
	heptaMarketTime(int hour, int minute, int second, int MilliSecond = 0, int StartTime = 6);
	heptaMarketTime(std::string updatetime, int MilliSecond = 0, int StartTime = 6);
	~heptaMarketTime();
	...
	bool operator > (heptaMarketTime const& right) const;
	bool operator < (heptaMarketTime const& right) const;
	bool operator >= (heptaMarketTime const& right) const;
	bool operator <= (heptaMarketTime const& right) const;
	bool operator == (heptaMarketTime const& right) const;
	bool operator != (heptaMarketTime const& right) const { return !operator ==(right); }
...
typedef std::shared_ptr<heptaHeptaTrader::heptaMarketTime> heptaMarketTimePtr;
