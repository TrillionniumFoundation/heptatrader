...
typedef std::shared_ptr<heptaKindleStick> heptaKindleStickPtr;

class heptaKindleStickSeries
{
public:
	enum heptaKindleSeriesType
	{
		heptaKindleTypeMinute = 0,							//分钟k线（允许秒k），k线周期级别按秒数来记
		heptaKindleTypeDaily,								//日线，				按交易日来记k线
	};
...
public:
	heptaKindleStickSeries();
	~heptaKindleStickSeries();

	//初始化K线  不需根据品种交易时间进行
	bool InitialKindleStickSeries(const char * szInstrumentID, heptaKindleSeriesType type = heptaKindleTypeMinute, uint32_t m_iTimeScale = c_1_minute);
	//初始化K线  需根据品种交易时间进行
	bool InitialKindleStickSeries(const char * szInstrumentID, const char * szProductID, 
		heptaKindleSeriesType type = heptaKindleTypeMinute, uint32_t iTimeScale = c_1_minute, const char * TradeTimeFile = "");

	//行情更新，调用后会自动形成k线
	void PriceUpdate(heptaMarketDataPtr pPriceData);

	//利用k线来更新K线序列。主要利用利用k线来合并数据,要求k线序列需要按顺序进行更新
	void UpdateKindle(heptaKindleStickPtr pKindle);
	//替换现有k线序列中的k线，依据StartTime这个字段进行判断
	void ReplaceKindle(heptaKindleStickPtr pKindle);
	//移除时间点之前的K线
...
public:
	heptaMarketDataPtr					m_PrePriceData;
	heptaKindleStickPtr				m_pCurrentKindleStick;
	heptaRangeOpenClose				m_heptaRangeOCMode;
...
private:
	...
	heptaKindleStickPtr				_pTmpPreKindleStick;

public:
	//获取K线序列
	bool			 GetKindleSerise(std::deque<heptaKindleStickPtr> &	KindleStickDeque);

	//按时间顺序获取k线，nCount为k线序列，最早的k线nCount为0
	heptaKindleStickPtr GetKindleStick(unsigned int nCount = 0);
	//按时间逆序获取k线，nCount为k线序列，最近的k线nCount为0
	heptaKindleStickPtr GetLastKindleStick(unsigned int nCount = 0);
	...
	*/
	...
	bool GetHighest(double& High, heptaKindleStick::KindlePriceType type);
	bool GetHighest(unsigned int nBegin, unsigned int nEnd, double& High, heptaKindleStick::KindlePriceType type);
	bool GetHighest(unsigned int nCount, double& High, heptaKindleStick::KindlePriceType type);
	...
	*/
	...
	bool GetLowest(double& Low, heptaKindleStick::KindlePriceType type);
	bool GetLowest(unsigned int nBegin, unsigned int nEnd, double& Low, heptaKindleStick::KindlePriceType type);
	bool GetLowest(unsigned int nCount, double& Low, heptaKindleStick::KindlePriceType type);
...
private:
	...
	heptaKindleSeriesType						m_heptaKindleSeriesType;
	//K线周期，秒为单位
...
#ifdef KINDLE_MULTI_THREAD
	heptaMUTEX									m_KindleQueueMutex;
#endif // KINDLE_MULTI_THREAD
	std::deque<heptaKindleStickPtr>			m_KindleStickDeque;
