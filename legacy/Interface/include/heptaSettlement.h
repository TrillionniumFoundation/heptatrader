...
#pragma endregion
	...
	//拓展更新函数
	void UpdatePrice(heptaMarketDataPtr pMdData, double dPriceTick, double dVolumeMultiple);

	void UpdateOrder(heptaOrderPtr pOrder);
	void UpdateTrade(heptaTradePtr pTrade, double dVolumeMultiple);
	...
	std::map<std::string, heptaOrderPtr>						m_OrderList;					//key OrderRef		value: orders
	std::map<std::string, heptaTradePtr>						m_TradeList;					//key TradeId		value: trades
