...
class heptaTickTradeManager
{
public:
	heptaTickTradeManager(bool bSimpleMode = false, bool bNeedStatic = false);
	~heptaTickTradeManager();

	void SetSimpleMode(bool bSimpleMode = false) { m_bSimpleMode = bSimpleMode; }
	void Reset();

	//update Instrument Data
	void SetInstrumentData(std::unordered_map<std::string, heptaInstrumentDataPtr>& InstrumentMap);

	//update Price values
	void UpdatePrice(heptaMarketDataPtr pPriceData);

	//Get this tick's trade volume
	std::map<int64_t, int64_t>& GetCurrentTradeMap() 
	{
		heptaAUTOMUTEX mt(m_CurrentTradeMutex, true);
	...
	std::map<uint32_t, uint32_t> GetLevelChange(heptaMarketDataPtr pPrePriceData, heptaMarketDataPtr pCurPriceData);					
private:
	std::unordered_map<std::string, heptaInstrumentDataPtr>	m_InstrumentMap;
	std::map<std::string, heptaMarketDataPtr>		m_PreMarketDataMap;

	//price update
	//Key:Price*1000 value:TradeVolume
	std::map<int64_t, int64_t>		m_CurrentTradeMap;
	heptaMUTEX							m_CurrentTradeMutex;
